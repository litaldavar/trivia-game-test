...
public class integrationTests extends TestUtilities {
	...
	public void facebookIntegrationTest()  {
		GameSteps steps = new GameSteps(driver);
		
		Reporter.log("Share game with facebook",true);
		...
		//Step 35: click share with Facebook	
		steps.shareWithFacebook();
		sleep(1000);
		Assert.assertTrue(steps.isAlert(), "Error message didn't show - can not share with Facebook");
		
		Assert.assertTrue(steps.clickOkonAlert() , "did not click ok on alert");
		
		sleep(3000);
		
		Assert.assertTrue(steps.confirmText("Facebook"),"Error: Facebook site did not open");
